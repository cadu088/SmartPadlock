...
namespace SmartPadlock.Classes
{
    internal class createUser
    {
        public bool Create(User usuario)
        {
            ...
            while (!verificaSenha)
            {
                Console.WriteLine("Digite uma senha unica de no minimo 10 caracteres para continuar: ");
                usuario.PASSWORD_USER = Console.ReadLine();
                verificaSenha = usuario.PASSWORD_USER.Length >= 10;
            }

            //criar user dps de verificar senha
            if (aplicationControler.CriptografarUser(new Contract(usuario, false), new Contract(usuario, true), usuario))
            {
                return true;
            }
            else
